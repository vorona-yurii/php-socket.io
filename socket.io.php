...
class SocketIOConnect
{
    ...
    private function handshake()
    {
        ...
        //$result = fread($this->socket, 100000);
        //var_dump($result);

        //preg_match('#Sec-WebSocket-Accept:\s(.*)$#mU', $result, $matches);
        //$keyAccept = trim($matches[1]);
        //$expectedResonse = base64_encode(pack('H*', sha1($key . '258EAFA5-E914-47DA-95CA-C5AB0DC85B11')));
        //return ($keyAccept === $expectedResonse);
    }

    /**
     * @param $action - action to execute in sockt server
     * @param $json - message to socket server
     * @return boolean
     */
    public function send($action = "message", $json = null)
    {
        if ($this->connect()) {
            sleep(1);
            fwrite($this->socket, $this->hybi10Encode('42["' . $action . '", ' . $json . ']'));
